...
## Open questions — RESOLVED (real-nix smoke)

Resolved by the real-nix smoke (throwaway `$HOME`, Determinate Nix 3.21.0), the way #81/#87 resolved theirs. One `homeManager.settings` block (curated `git` + curated `shell.aliases` + raw `programs.bash` + a `files` entry) compiled → generated flake → activated cleanly; every path was observable.

- **Curated→home-manager mappings — CONFIRMED.** `git` → `programs.git.enable` + the STABLE `programs.git.extraConfig` (smoke produced a correct `~/.config/git/config` with `[user] name` + `[init] defaultBranch` and **no deprecation warning** — extraConfig sidesteps the `programs.git.userName` → `programs.git.settings.user.name` churn #87 hit, proving the curated layer's insulation value). `shell.aliases`/`sessionVariables` → `home.shellAliases`/`home.sessionVariables` (the alias landed in `~/.bashrc` once `programs.bash` was enabled). `direnv`/`starship` → `programs.<x>.enable`. Raw `programs` forwarded verbatim (bash activated).
- **`files`: `home.file` uniformly — CONFIRMED (no `xdg.configFile` in v1).** Targets are normalized home-relative (strip leading `~/`), staged into `<flakedir>/files/<sanitized>` (binary-safe), emitted as `home.file."<rel>".source = ./files/<sanitized>`. The smoke placed `~/.config/endstate-smoke.txt` correctly (store symlink), so one `home.file` path covers `.config/...` and other home-relative targets; XDG inference is unnecessary complexity for v1.
- **flake.lock — same verdict as #87:** the engine does not generate one; Nix auto-writes its own lock into the persistent generated dir on activation (a 1234-byte `flake.lock` appeared). The dir is a complete, **ejectable** flake — a hand-run `nix run home-manager -- switch --flake <gen dir>#<user>` succeeded (exit 0).
