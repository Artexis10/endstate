...
- [x] 1B.2 Add RED tests over the v0 evidence shapes for causal failure extraction, typed lane assembly, source/toolchain repetition identity, infrastructure precedence, and strict aggregate inventory; implement the minimal v1 controller until they pass.
- [x] 1B.3 Replace the branch workflow with the typed v1 controller and three-OS Go comparator; bind one exact resolved Go patch version on every runner and prohibit integration, target installation, inline JSON composition, ambient authority, workflow inputs, and reruns. Independently review the detector, controller, and operative workflow.
- [x] 1B.3A Amend candidate metadata and patch eligibility for exactly one file in the closed lifecycle registry; reject the frozen Windows-Go target registry; bind each lifecycle to the loaded production sidecar mode; require each detector attempt to co-build mutated `endstate` and `endstate-validation` binaries and invoke the validation CLI; add focused RED tests for every newly allowed/forbidden scope, metadata/review identity, target overlap, sidecar-mode binding, the closed credit table, `already-covered` comparator classification, and rejection of a frozen in-process oracle; independently review the change and create a new proof-machinery freeze before authoring exact mutants.
- [ ] 1B.4 Author and independently review three operator- and invariant-disjoint held-out module-lifecycle product regressions spanning `capture-contract` and `config-roundtrip-v1`, with at least one in each verified sidecar mode, without executing them through a detector; allow only corpus files after the new proof-machinery freeze and mechanically verify the restricted repository diff.
