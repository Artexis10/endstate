...
- [ ] 9.7 Add disposable-runner lifecycle and cleanup proof: observe clean absence immediately before install receipt and presence immediately after, seed only staged hash-bound bytes with least privilege, use unique attempt roots and link-safe exact-leaf wipe, bound cleanup, and retry only after clean absence is re-proven. UI, elevation, reboot, and lock conditions are unsupported and not retryable.
- [ ] 9.8 Introduce, in a later change, the external trusted exact-ref hosted authority that may mint mutation eligibility and promote a candidate only after a real fresh trusted hosted success. Do not mark hosted/public proof complete until that run exists.
