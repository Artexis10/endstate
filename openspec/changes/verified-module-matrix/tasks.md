...
- [ ] 10.9 Generalize install-contract fixtures across the exact production `command-exists`, `file-exists`, and key-only `registry-key-exists` verifier families while preserving negative-before-positive verification and `catalog + engine-contract` proof only.
