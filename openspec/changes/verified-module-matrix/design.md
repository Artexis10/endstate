...
The first real-app lane is a dedicated `.github/workflows/hosted-live.yml` workflow, separate from the fast synthetic `go-ci.yml` matrix and its stable aggregate. It runs the fixed `Notepad++.Notepad++` candidate on trusted `main` through `schedule` and `workflow_dispatch`; it does not run candidate mutations from pull requests. A protected qualification-campaign record pins the exact checkout commit, engine binary hash, module/sidecar/seed/comparator identity, package policy, observer boundary, and workflow/policy digest. Scheduled attempts keep testing that pinned campaign even while unrelated commits move `main`; changing any pinned field starts a new streak.
...
- Scheduled matrices continue after individual failures so the aggregate shows the complete state. The final aggregator fails when required evidence is missing or malformed, a failure is not covered by protected-main debt/quarantine authority, or any other gate invariant is violated. Authorized debt and quarantine remain visibly failed and outside every pass numerator.

### Synthetic known-failure debt

The initial all-catalog matrix is intentionally honest about its residual debt. A known-failure ledger does not convert a failed scenario into proof: the summary continues to report the real numerator, for example `261 / 362` passing scenarios plus `101` known failures. The ledger exists only to distinguish already-reviewed debt from a regression so ordinary pull requests can have one meaningful stable gate.

Each ledger row is keyed by the stable logical identity `moduleId + scenarioId + kind`. It stores the complete previous evidence identity and a structured failure fingerprint for audit, but mutable `moduleRevision`, scenario digest, and row hash are not matching keys. The aggregate independently reconstructs and validates those full head identities on every run. Diagnostic prose is informational; gating uses stable structured failure fields. A changed fingerprint is an explicit transition requiring maintainer review, never a silent edit.

The ledger header binds the complete logical row inventory, total module count, total scenario count, and canonical inventory SHA-256. Pull requests may add scenarios but may not silently remove or rename an inventory row. A legitimate removal requires an explicit reviewed inventory transition. A passing row may not remain in the known-failure set; fixing a scenario requires deleting its debt entry. A newly failing logical row, incomplete or malformed evidence, an unreviewed failure transition, a stale retained entry, or an unauthorized inventory removal fails the aggregate.

On pull requests, the checked-out merge commit must have exactly two parents. Checkout fetches those parents without persisting credentials; the workflow verifies the base parent and extracts only its committed ledger blob into runner-temporary storage. The head ledger may only remove resolved entries or consume a narrowly structured, one-use transition already authorized by the base. Workflow and verifier changes remain protected by ordinary code review and branch protection: the ledger prevents accidental or casual self-rebaselining, but is not represented as tamper-proof against a reviewer-approved rewrite of the gate itself.

Synthetic shard jobs are evidence producers. A shard exits successfully when it emits complete canonical evidence for every assigned row, including genuine failed results. Setup, execution, identity, serialization, or evidence-integrity errors still fail the shard. The stable `Verified Module Matrix` aggregate is the only regression gate and remains red for unknown debt, malformed/missing evidence, catalog/canary failure, or unauthorized inventory/fingerprint changes. Known failures remain visibly failed in evidence and never enter a proof numerator.

There is no generic bootstrap exception. PR #205 may merge while the new aggregate remains non-required and red on known residuals. Its first protected-main run establishes the exact row evidence. A separate baseline-only reviewed change then seeds the protected-main ledger; that seed pull request is expected to remain red because its base has no authority to add debt. Once the reviewed ledger is on protected main, a fresh run must produce a green aggregate before branch protection requires it. This one-time rollout cannot be reused by later pull requests.

### Validation metadata maintenance and reduction

Validation sidecars remain one-per-module and every current scenario remains explicit. Only uniform boilerplate is defaulted: automatic fixture type, the 120-second timeout, minimum assertions derived from scenario kind plus production-module verifier presence, and the two stable live reason/explanation pairs derived from live mode. Declarative fixture paths/hashes, scenario identity/mode, schema-v2 expected identities, one-way reviews, all six intentional `verify: 2` overrides, and live mode remain authored.

Parsing is presence-aware. An omitted defaultable field receives its default; an explicitly empty or zero value is rejected. A supplied minimum-assertion object is a complete override rather than a partial merge. Defaults resolve with the production module available and before validation, canonical scenario digesting, shard assignment, or row identity. Before rewriting sidecars, a golden migration comparison must prove that all 359 expanded records, all 362 logical rows, scenario/live digests, shard assignments, and row identities are unchanged. Raw validation-source hashes may change once because the canonical source bytes become smaller.

The validator provides `sync-revisions --repo <path>` in check-only mode and an explicit `--write` mode. It reuses the production module revision calculation, preflights every sidecar before writing, replaces exactly one standard revision token atomically while preserving all other bytes and line endings, reloads the catalog afterward, and never edits the known-failure ledger. This removes routine pin drift without creating a new authority.

Reduction stops at independent-oracle boundaries. Duplicate strict-JSON scanning may be consolidated inside the harness, but the persisted cross-job evidence scanner in `validationci` stays independent. Production restore traversal helpers may be shared only through the existing I/O boundary with characterization tests for native and guarded call order, link rejection, deterministic order, `SkipDir`, preflight-before-mutation, and coordinates. Harness oracles, schema journeys, the Notepad++ canary, declarative fixtures, and hosted-live foundations are not removed for line-count optics. The tracked `.superpowers` report is process debris and is removed from the repository with an anchored ignore rule.
...
7. Keep the synthetic aggregate non-required through the initial protected-main evidence run; seed the exact reviewed debt ledger in a separate baseline-only change, require a subsequent protected-main green run, and only then enable the stable gate.
