...
The engine SHALL fail the push when the required commit request fails for any
reason, including an absent endpoint reported as 404, with an actionable error
naming the backup and version. The engine SHALL NOT report the generation as
protected.

#### Scenario: Backend error on commit fails the push

- **WHEN** the commit request returns any non-2xx status, including 404
