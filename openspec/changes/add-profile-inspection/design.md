...
Every owned module selects exactly one verified owner-ref tier: non-empty `configCaptures[].sourceInstance.evidence.ref` values first; then package refs from successfully verified embedded snapshots; then trusted-catalog package refs for that already-owned module. The first non-empty tier wins. Lower tiers can provide labels but cannot add association candidates or override a higher tier. Within the selected tier, refs are trimmed, case-insensitively deduplicated, and sorted. A failed verification of an intended higher tier emits the contracted warning and allows the next tier.

Apps inventory rows receive their opaque identities before presentation sorting: `app:<case-folded-manifest-app-id-or-unnamed>:<one-based-occurrence-among-that-case-folded-id-in-resolved-manifest-order>`. Each row preserves its manifest identifier separately. A candidate is an Apps row whose normalized `packageRefs[]` intersects a selected owner-ref set by exact case-insensitive equality. Exactly one candidate is `included`, multiple candidates are `ambiguous`, zero candidates with selected refs is `not_in_profile`, and no selected refs is `unresolved`.

Every `included` module sharing one Apps row is grouped, and every `not_in_profile` module sharing the complete canonical absent-owner key is grouped; `ambiguous` and `unresolved` modules are never grouped. The absent-owner key is `package:` plus the complete sorted case-folded selected-ref set joined by `|`. Association field semantics are fixed: `ownerId` is non-null only for included/not-in-profile, `appId` only for included, `appIncluded` is true iff included, and `candidateAppIds` is the single Apps row ID for included, sorted Apps row IDs for ambiguous, and empty otherwise. Settings row IDs are `settings:<app-row-id>` for included, `settings:<absent-owner-key>` for not-in-profile, and `settings:module:<canonical-module-key>` for ambiguous/unresolved. An included `ownerId` equals the Apps row ID. Only included marks that Apps row `hasSettings`.

Apps use captured `displayName`, then the first sorted package ref, then humanized manifest app ID; package refs are all non-empty `refs` values, trimmed, deduplicated, and sorted. Settings use verified snapshot `displayName`, trusted-catalog `displayName`, associated app `displayName`, first sorted verified package ref, then a humanized canonical module key. `capturedAt` is non-empty manifest `captured`, then sibling metadata `capturedAt`, then null; conflicts can warn diagnostically. Apps and settings sort by case-folded display name then row ID; inner arrays sort by case-folded value then original value; warnings sort by code then message. Arrays are never null and summaries derive after grouping.
