...
- [x] 9.1 Convert one stable-layout module to schema v2 with one generation and prove direct restore remains simple.
- [x] 9.2 Convert one versioned-directory module to path instance discovery and prove side-by-side capture/selection works.
...
- [x] 10.1 Run targeted Go tests for modules, bundle, manifest, planner/resolution, migration operations, restore/journal/revert, commands, envelopes, and events.
- [x] 10.2 Run OpenSpec strict validation and contract/schema fixture validation.
...
- [x] 10.4 Run the full Go test suite after targeted suites pass and record any environment-specific exclusions.
...
- [x] 10.6 Merge `origin/main` at `9dc00c8cc29f9768a9cac880267624a09f90e7cb` into the config-generation branch in a disposable integration workspace, resolve and record inspection of the 16 known conflicts, and manually inspect the 10 shared files that Git auto-merges.
- [x] 10.7 Generalize package generation capture, instance matching, post-install re-detection, and diagnostics across every selected engine-supported driver using driver-qualified package identity; retain backward-compatible `configModuleMap`, populate driver-qualified `packageModuleMap`, and add a Chocolatey `g1` -> `g2` regression proving Chocolatey evidence on capture and final detection, case-insensitive Chocolatey ref canonicalization, and no Winget fallback.
- [x] 10.8 Add `TestConfigGenerationCaptureMigrationRestoreRevertRoundTrip` using `RunCapture` and real restore/revert surfaces to prove bundle-v2 hash stability, the explicit migration path, stdout envelope, stderr JSONL ordering, committed journal lineage, and exact revert bytes; add `scripts/smoke/config-generation-windows-terminal.ps1` with a Windows-Terminal-only temporary catalog, closed-process precondition, independently hash-verified backup, final original-hash/sentinel checks, and retained backup on verification failure so no Alacritty config path is resolved or modified.
- [x] 10.9 Re-run targeted and full Go tests, vet, build, OpenSpec validation, Windows/Linux verification, macOS CI, and an independent final review against the combined tree.
- [x] 10.10 Release engine read support and structured contracts before enabling GUI v2 capture/restore; retain v2 read support if capture must be rolled back.
