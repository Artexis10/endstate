...
- [x] 1.1 `internal/realizer/realizer.go`: add optional `HomeRollbacker { RollbackHome(generation int) (newGen int, err error) }`
      with a doc-comment in the `Pruner` / `HomeActivator` style (type-asserted, optional).

## 2. Nix backend RollbackHome

- [x] 2.1 `internal/realizer/nix/nix.go`: add an injectable script-exec seam to `Backend` (e.g.
...
- [x] 2.2 RED tests (`internal/realizer/nix/home_rollback_test.go`): `RollbackHome(M)` resolves
...
- [x] 2.3 Implement `(*Backend).RollbackHome` (new `home_rollback.go` or in `home_manager.go`): reuse
...
- [x] 3.1 `internal/commands/rollback.go`: add `EnableRestore bool` to `RollbackFlags`; add a
      `HomeManager *RollbackHomeResult` sub-object to `RollbackResult` (target hm gen, new hm gen, flake/config,
      dry-run-aware).
- [x] 3.2 `appendRollbackGeneration` (apply_generation.go or local): accept an optional `*provision.HomeGenRef`
      so the appended rollback generation records the now-active config.
- [x] 3.3 RED command tests + impl in `runRealizerRollback`: validate config eligibility BEFORE mutating
...
- [x] 3.4 Tests: coupled success records `HomeManager.Generation == newHmGen` and rolls packages too; no
...
- [x] 3.5 `TestRollbackStaysPackageOnly` stays green — `rollback.go` MUST NOT import `internal/restore`.

## 4. CLI wiring (PROTECTED — additive)

- [x] 4.1 `cmd/endstate/main.go`: add `EnableRestore: p.enableRestore` to the `rollback` case; update the
...
- [x] 5.2 `GOOS=windows go build ./...` + `go vet ./...` clean (realizer/home-manager path is non-Windows;
      winget/default rollback untouched).
- [x] 5.3 `npm run openspec:validate` (strict) passes.
- [x] 5.4 **Real-nix e2e smoke** (sandbox `$HOME` + isolated `ENDSTATE_ROOT`; hm pin
