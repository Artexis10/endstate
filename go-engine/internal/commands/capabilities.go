...
func RunCapabilities() (interface{}, *envelope.Error) {
	data := CapabilitiesData{
		...
		Commands: map[string]CommandInfo{
			...
			"apply": {
				Supported: true,
				Flags:     []string{"--manifest", "--dry-run", "--enable-restore", "--restore-filter", "--restore-target", "--only", "--bootstrap-backends", "--no-bootstrap", "--json", "--events"},
			},
			"verify": {
				Supported: true,
			...
			"capture": {
				Supported: true,
				Flags:     []string{"--profile", "--out", "--name", "--sanitize", "--discover", "--update", "--include-runtimes", "--include-store-apps", "--exclude-store-apps", "--minimize", "--manifest", "--json", "--events", "--pin", "--driver", "--only", "--share"},
			},
			"plan": {
				Supported: true,
				Flags:     []string{"--manifest", "--json", "--events"},
