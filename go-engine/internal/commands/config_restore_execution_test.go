...
func TestConfigRestoreExecutionLegacyNoopPreservesExistingRestoreState(t *testing.T) {
	root := t.TempDir()
	...
	target := filepath.Join(root, "target", "settings.json")
	for path, content := range map[string]string{source: "{\"theme\":\"dark\"}\n", target: "{\"theme\":\"dark\"}\n"} {
	...
	priorJournal, err := writeLegacyConfigRestoreJournal(configRestoreExecutionOptions{
		RunID: "prior-restore", JournalLogsDir: logsDir, ManifestDir: manifestDir,
	}, []restore.RestoreResult{{ID: "prior", Source: source, Target: target, Status: "restored", RestoreType: "copy"}})
	if err != nil {
		t.Fatal(err)
	}
	priorGuard, err := configrestore.BeginLive(context.Background(), stateDir, "prior-restore", nil)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := priorGuard.RegisterLegacyJournal(priorJournal); err != nil {
		_ = priorGuard.Close()
		t.Fatal(err)
	}
	if err := priorGuard.Close(); err != nil {
		t.Fatal(err)
	}
	before := map[string]map[string]configRestoreTreeEntry{
		"logs":                 snapshotConfigRestoreTree(t, logsDir),
		"state/config-restore": snapshotConfigRestoreTree(t, filepath.Join(stateDir, "config-restore")),
		"state/backups":        snapshotConfigRestoreTree(t, filepath.Join(stateDir, "backups")),
	}
	...
	inputs.legacyLanes = []configRestoreLegacyLane{{
		captureID: bundle.LegacyCaptureID("apps.legacy"), moduleID: "apps.legacy", configSetID: "legacy",
		restoreEntries: []manifest.RestoreEntry{{Type: "copy", Source: "settings.json", Target: target, Backup: true, FromModule: "apps.legacy"}},
		selected:       true,
	}}
	buffer := &bytes.Buffer{}
	session := &configRestoreExecutionSession{
	...
	result, envErr := session.Execute(context.Background(), configRestoreExecutionOptions{
		RestoreEnabled: true, RunID: "noop-restore", StateDir: stateDir, ManifestDir: manifestDir, JournalLogsDir: logsDir,
		BackupDir: filepath.Join(stateDir, "backups", "noop-restore"), Emitter: events.NewEmitterWithWriter("noop-restore", true, buffer),
	})
	if envErr != nil {
		t.Fatalf("Execute() error = %+v", envErr)
	}
	if len(result.RestoreItems) != 1 || result.RestoreItems[0].Status != "skipped_up_to_date" ||
		result.RestoreItems[0].BackupCreated || result.RestoreItems[0].BackupPath != "" || result.JournalPath != "" {
		t.Fatalf("restore result = %+v", result)
	}
	if len(result.Plan.Sets) != 1 || result.Plan.Sets[0].Resolution.Status != planner.StatusSkipped ||
		result.Plan.Sets[0].Resolution.Reason == nil || *result.Plan.Sets[0].Resolution.Reason != planner.ReasonAlreadyUpToDate {
		t.Fatalf("plan = %+v", result.Plan)
	}
	for name, path := range map[string]string{
		"logs":                 logsDir,
		"state/config-restore": filepath.Join(stateDir, "config-restore"),
		"state/backups":        filepath.Join(stateDir, "backups"),
	} {
		if after := snapshotConfigRestoreTree(t, path); !reflect.DeepEqual(after, before[name]) {
			t.Fatalf("%s changed during no-op restore:\n before=%#v\n after=%#v", name, before[name], after)
		}
	}
	...
	for _, line := range strings.Split(strings.TrimSpace(buffer.String()), "\n") {
		...
		if event["event"] == "restore-item" && event["status"] == "skipped" && event["reason"] != planner.ReasonAlreadyUpToDate.String() {
	...
	if terminal == nil || terminal["skipped"] != float64(1) || terminal["total"] != float64(1) {
		t.Fatalf("summary = %#v", terminal)
	}
}

type configRestoreTreeEntry struct {
	Mode    os.FileMode
	ModTime time.Time
	Content []byte
}

func snapshotConfigRestoreTree(t *testing.T, root string) map[string]configRestoreTreeEntry {
	t.Helper()
	entries := map[string]configRestoreTreeEntry{}
	...
	entries["."] = configRestoreTreeEntry{Mode: info.Mode(), ModTime: info.ModTime()}
	if err := filepath.Walk(root, func(path string, info os.FileInfo, walkErr error) error {
		...
		entry := configRestoreTreeEntry{Mode: info.Mode(), ModTime: info.ModTime()}
