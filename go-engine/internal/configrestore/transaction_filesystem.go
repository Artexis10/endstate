...
func executeTransactionAction(
	...
	if action.Kind != ActionRegistrySet {
		if err := createMissingTransactionParents(ctx, journal.MissingParents, touch); err != nil {
			return err
		}
	}
	switch action.Kind {
	case ActionCopy:
		return commitCopyAction(ctx, action, journal, touch)
	case ActionWriteFile:
		...
		return atomicWriteTransactionFile(ctx, action.Target, action.DesiredContent, desired.Mode, touch)
	case ActionDeleteFile:
		return removeTransactionFile(ctx, action.Target, touch)
	case ActionRegistrySet:
		return commitRegistryAction(ctx, action, journal, registry, touch)
	default:
		return fmt.Errorf("unsupported transaction action %q", action.Kind)
	}
}

func commitCopyAction(ctx context.Context, action Action, journal JournalAction, touch func()) error {
	...
	case StateFile:
		...
		if err := prepareTransactionFileDestination(ctx, action.Target, touch); err != nil {
			return err
		}
		if err := atomicWriteTransactionFile(ctx, action.Target, data, desired.Mode, touch); err != nil {
			return err
		}
	case StateDirectory:
		if err := commitDirectoryCopy(ctx, action, source, desired, touch); err != nil {
			return err
		}
	default:
...
func commitDirectoryCopy(
	...
	if err := ensureTransactionDirectory(ctx, action.Target, rootEntry.Mode, touch); err != nil {
		return err
	}
	...
	for _, relative := range paths {
		...
		case StateDirectory:
			if err := ensureTransactionDirectory(ctx, targetPath, desiredEntry.Mode, touch); err != nil {
				return err
			}
		case StateFile:
			...
			if err := prepareTransactionFileDestination(ctx, targetPath, touch); err != nil {
				return err
			}
			if err := atomicWriteTransactionFile(ctx, targetPath, data, desiredEntry.Mode, touch); err != nil {
				return err
			}
		default:
...
func prepareTransactionFileDestination(ctx context.Context, target string, touch func()) error {
...
func atomicWriteTransactionFile(
	...
	return replaceTransactionFile(temporaryPath, destination)
}
...
func ensureTransactionDirectory(
	...
	if os.IsNotExist(err) {
		...
		return syncDurableDirectory(parent)
	}
	if err != nil {
		return err
	}
	if !info.IsDir() || isLinkOrReparse(info) {
		...
		return ensureTransactionDirectory(ctx, path, mode, touch)
	}
...
func removeTransactionFile(ctx context.Context, target string, touch func()) error {
	if err := checkSnapshotContext(ctx); err != nil {
		return err
	}
	if err := rejectExistingTargetLinks(target); err != nil {
...
func restoreFilesystemPrior(
	...
	case StateFile:
		...
		return atomicWriteTransactionFile(ctx, target, data, prior.Mode, nil)
	case StateDirectory:
		backup, err := scanFilesystemState(ctx, backupPath)
		if err != nil {
...
func restoreRecordedDirectory(
	...
	for _, relative := range paths {
		...
		switch entry.Kind {
		case StateDirectory:
			if err := ensureTransactionDirectory(ctx, target, entry.Mode, nil); err != nil {
				return err
			}
		case StateFile:
			...
			if err := prepareTransactionFileDestination(ctx, target, nil); err != nil {
				return err
			}
			if err := atomicWriteTransactionFile(ctx, target, data, entry.Mode, nil); err != nil {
				return err
			}
		default:
