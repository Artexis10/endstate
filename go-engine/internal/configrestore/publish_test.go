...
func TestPublicationFailureDistinguishesProvenAbsenceFromAmbiguity(t *testing.T) {
	...
	if got := publicationFailure(publicationNotDurable, publishErr, os.ErrNotExist); !errors.Is(got, publishErr) ||
	...
	if got := publicationFailure(publicationAmbiguous, publishErr, reconcileErr); !errors.Is(got, publishErr) ||
