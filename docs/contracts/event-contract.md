...
`endstate rebuild` does **not** define new event types. It composes the **existing** `apply` and `verify` event streams unchanged: apply's stream (opening with a `phase` event, closing with a `summary`) is followed by verify's stream (also opening `phase`, closing `summary`). The concatenation still satisfies the ordering invariant — the first event is a phase and the last is a summary. Schema stays **v1**. (Known cosmetic divergence: the streamed sub-run `runId`s are `apply-<ts>`/`verify-<ts>` while the `rebuild` JSON envelope carries its own `rebuild-<ts>` — unifying them is a deferred GUI-facing follow-up.)
