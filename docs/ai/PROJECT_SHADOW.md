...
9. **`-EnableRestore` must be wired through `Invoke-ApplyCore`.** The flag is parsed by the CLI entrypoint but must be explicitly passed to `Invoke-ApplyCore` and used to trigger restore processing after the app install loop. Without this, restore entries in the manifest are silently ignored.
10. **Capture must inject module restore entries into zip manifest.** `New-CaptureBundle` stages config files under `configs/<module-id>/` via `Invoke-CollectConfigFiles`, but the staged `manifest.jsonc` starts with an empty `restore` array. Stage 2b in `New-CaptureBundle` must inject cloned restore entries from matched modules into the staged manifest before zipping.
11. **Restore source paths must be rewritten to match zip layout.** Module definitions use `./payload/apps/<id>/` source paths but the zip stages files under `configs/<id>/`. Stage 2b rewrites `$clone.source` to `./configs/<moduleDirName>/<leaf>` after cloning. Without this, all restores except those already using `./configs/` (e.g. msi-afterburner) fail with "Source not found".
12. **PowerShell `Copy-Item -Recurse` nests directories.** When the destination directory already exists, `Copy-Item -Recurse` copies the source *inside* the dest, creating `Profiles/Profiles/`. Both `Invoke-ConfigModuleCapture` and `Invoke-CollectConfigFiles` must `Remove-Item $destPath -Recurse -Force` before `Copy-Item` for directory sources to ensure idempotent captures.
13. **`Invoke-ApplyCore` restore loop bypasses engine.** The CLI entrypoint's `Invoke-ApplyCore` (in `bin/endstate.ps1`) has its own restore loop separate from `engine/apply.ps1` (`Invoke-Apply`) and `engine/restore.ps1` (`Invoke-Restore`). Any restore-related feature added to the engine functions (journaling, new restorer types, event emission) must also be added to `Invoke-ApplyCore`, or the CLI apply path will silently lack that feature.
