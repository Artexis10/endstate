...
3. **No automatic version management** — the engine never *silently or automatically* changes a package's version. Version handling is **explicit and opt-in**, only on backends that support versioned install (the winget driver; the Nix realizer pins via its ref and ignores per-app versions): a package is installed at a specific version only when the manifest declares `version`; `verify` compares the declared version against the installed one and reports a read-only `version_drift` failure on mismatch; and `apply --repin --confirm` reinstalls the declared version over a drifted one. Default `apply` is **pin-on-install only** — without `--repin --confirm` it never downgrades or reinstalls an already-present package to match a declared version. The installed version is recorded best-effort in the Provisioning Generation.
