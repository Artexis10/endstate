...
2. **No forced cross-platform symmetry** — backend behavior follows each platform's package-management model: Windows defaults to Winget and supports explicit Chocolatey lanes, Linux uses the Nix realizer, and macOS composes Nix with an explicit Brew lane. Capabilities may differ by backend; unsupported drivers remain visible and are never silently substituted.
3. **No automatic version management** — the engine never *silently or automatically* changes a package's version. Version handling is **explicit and opt-in**, only on backends that support it (Winget and Chocolatey support per-app versioned install/repin; the Nix realizer pins through its ref and ignores per-app versions): a package is installed at a specific version only when the manifest declares `version`; `verify` compares the declared version against the installed one and reports a read-only `version_drift` failure on mismatch; and `apply --repin --confirm` reinstalls the declared version over a drifted one. Default `apply` is **pin-on-install only** — without `--repin --confirm` it never downgrades or reinstalls an already-present package to match a declared version. The installed version is recorded best-effort in the backend-scoped Provisioning Generation.
4. **No automatic rollback** — failed operations do not auto-rollback. All rollback is explicit and opt-in: manual `revert` reverses a restore (configuration), and the manual, `--confirm`-gated `rollback` command reverts package changes using native Nix rollback or backend-authoritative best-effort uninstall for supported package drivers (Winget, Chocolatey, and Brew). Mixed-driver applies are grouped by run ID so one apply remains one rollback unit. Neither rollback path runs automatically.
